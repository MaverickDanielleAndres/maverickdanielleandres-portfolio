...
const GooeyNav: React.FC<GooeyNavProps> = ({
  ...
  // ✅ keep in sync if the parent changes the prop
  useEffect(() => {
  ...
  useEffect(() => {
    if (!pathname) return;
    const idx = items.findIndex(i => i.href && i.href !== "#" && i.href === pathname);
  ...
  useEffect(() => {
    ...
    if (activeLi) {
      ...
      // Trigger initial animation only once after component mounts
      if (!isInitialized && filterRef.current) {
        setTimeout(() => {
          if (filterRef.current) {
            makeParticles(filterRef.current);
          }
  ...
  .effect.text.active {
    color: black; /* gray instead of black */
  ...
  li {
    position: relative;
    color: #d4d4d4; /* default light gray */
    font-weight: 500;
    transition: color 0.3s ease;
  }
  li:hover {
